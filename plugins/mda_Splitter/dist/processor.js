import createMDA_SplitterPlugin from './MDA_Splitter.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"mode":"_shim_set_mode","freq":"_shim_set_freq","freq_sw":"_shim_set_freq_sw","level":"_shim_set_level","level_sw":"_shim_set_level_sw","envelope":"_shim_set_envelope","output":"_shim_set_output"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_SplitterPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_Splitter', WadspProcessor);
