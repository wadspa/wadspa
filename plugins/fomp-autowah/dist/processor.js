import createAutowahPlugin from './Autowah.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0];
const SETTERS = {"drive":"_shim_set_drive","decay":"_shim_set_decay","range":"_shim_set_range","freq":"_shim_set_freq","mix":"_shim_set_mix"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createAutowahPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-Autowah', WadspProcessor);
