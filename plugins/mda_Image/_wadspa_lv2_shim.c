...
static float g_ctrl_s_width = 100.0f;
static float g_ctrl_s_pan = 0.0f;
static float g_ctrl_m_level = 100.0f;
static float g_ctrl_m_pan = 0.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/Image";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 1, &g_ctrl_s_width);
    g_desc->connect_port(g_handle, 2, &g_ctrl_s_pan);
    g_desc->connect_port(g_handle, 3, &g_ctrl_m_level);
    g_desc->connect_port(g_handle, 4, &g_ctrl_m_pan);
    g_desc->connect_port(g_handle, 5, &g_ctrl_output);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_s_width(float v) { g_ctrl_s_width = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_s_width()        { return g_ctrl_s_width; }
EMSCRIPTEN_KEEPALIVE void  shim_set_s_pan(float v) { g_ctrl_s_pan = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_s_pan()        { return g_ctrl_s_pan; }
EMSCRIPTEN_KEEPALIVE void  shim_set_m_level(float v) { g_ctrl_m_level = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_m_level()        { return g_ctrl_m_level; }
EMSCRIPTEN_KEEPALIVE void  shim_set_m_pan(float v) { g_ctrl_m_pan = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_m_pan()        { return g_ctrl_m_pan; }
