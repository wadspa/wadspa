import createMDA_ImagePlugin from './MDA_Image.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"mode":"_shim_set_mode","s_width":"_shim_set_s_width","s_pan":"_shim_set_s_pan","m_level":"_shim_set_m_level","m_pan":"_shim_set_m_pan","output":"_shim_set_output"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_ImagePlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_Image', WadspProcessor);
