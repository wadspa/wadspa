...
static float g_ctrl_headroom = -6.0f;
static float g_ctrl_quant = 10.0f;
static float g_ctrl_rate = 48000.0f;
static float g_ctrl_integrator_sr = 0.0f;
static float g_ctrl_post_filt = 15000.0f;
static float g_ctrl_non_lin = 0.16f;
static float g_ctrl_even_odd = 1.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/Degrade";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 0, &g_ctrl_headroom);
    g_desc->connect_port(g_handle, 1, &g_ctrl_quant);
    g_desc->connect_port(g_handle, 2, &g_ctrl_rate);
    g_desc->connect_port(g_handle, 3, &g_ctrl_integrator_sr);
    g_desc->connect_port(g_handle, 4, &g_ctrl_post_filt);
    g_desc->connect_port(g_handle, 5, &g_ctrl_non_lin);
    g_desc->connect_port(g_handle, 6, &g_ctrl_even_odd);
    g_desc->connect_port(g_handle, 7, &g_ctrl_output);
    g_desc->connect_port(g_handle, 8, g_in_left_in);
    g_desc->connect_port(g_handle, 9, g_in_right_in);
    g_desc->connect_port(g_handle, 10, g_out_left_out);
    g_desc->connect_port(g_handle, 11, g_out_right_out);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_headroom(float v) { g_ctrl_headroom = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_headroom()        { return g_ctrl_headroom; }
EMSCRIPTEN_KEEPALIVE void  shim_set_quant(float v) { g_ctrl_quant = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_quant()        { return g_ctrl_quant; }
EMSCRIPTEN_KEEPALIVE void  shim_set_rate(float v) { g_ctrl_rate = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_rate()        { return g_ctrl_rate; }
EMSCRIPTEN_KEEPALIVE void  shim_set_integrator_sr(float v) { g_ctrl_integrator_sr = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_integrator_sr()        { return g_ctrl_integrator_sr; }
EMSCRIPTEN_KEEPALIVE void  shim_set_post_filt(float v) { g_ctrl_post_filt = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_post_filt()        { return g_ctrl_post_filt; }
EMSCRIPTEN_KEEPALIVE void  shim_set_non_lin(float v) { g_ctrl_non_lin = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_non_lin()        { return g_ctrl_non_lin; }
EMSCRIPTEN_KEEPALIVE void  shim_set_even_odd(float v) { g_ctrl_even_odd = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_even_odd()        { return g_ctrl_even_odd; }
