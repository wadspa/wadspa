...
static float g_ctrl_level = 0.71f;
static float g_ctrl_channel = 0.5f;
static float g_ctrl_f1 = 0.57f;
static float g_ctrl_f2 = 0.5f;
static float g_ctrl_sweep = 0.3f;
static float g_ctrl_thru = 0.0f;
static float g_ctrl_zero_db = 1.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/TestTone";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 1, &g_ctrl_level);
    g_desc->connect_port(g_handle, 2, &g_ctrl_channel);
    g_desc->connect_port(g_handle, 3, &g_ctrl_f1);
    g_desc->connect_port(g_handle, 4, &g_ctrl_f2);
    g_desc->connect_port(g_handle, 5, &g_ctrl_sweep);
    g_desc->connect_port(g_handle, 6, &g_ctrl_thru);
    g_desc->connect_port(g_handle, 7, &g_ctrl_zero_db);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_f1(float v) { g_ctrl_f1 = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_f1()        { return g_ctrl_f1; }
EMSCRIPTEN_KEEPALIVE void  shim_set_f2(float v) { g_ctrl_f2 = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_f2()        { return g_ctrl_f2; }
EMSCRIPTEN_KEEPALIVE void  shim_set_sweep(float v) { g_ctrl_sweep = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_sweep()        { return g_ctrl_sweep; }
EMSCRIPTEN_KEEPALIVE void  shim_set_thru(float v) { g_ctrl_thru = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_thru()        { return g_ctrl_thru; }
EMSCRIPTEN_KEEPALIVE void  shim_set_zero_db(float v) { g_ctrl_zero_db = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_zero_db()        { return g_ctrl_zero_db; }
