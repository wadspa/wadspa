...
static float g_ctrl_mod_in = 0.0f;
static float g_ctrl_output = 0.0f;
static float g_ctrl_hi_thru = 40.0f;
static float g_ctrl_hi_band = 40.0f;
static float g_ctrl_envelope = 1602.24f;
static float g_ctrl_filter_q = 55.0f;
static float g_ctrl_mid_freq = 1000.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/Vocoder";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 0, &g_ctrl_mod_in);
    g_desc->connect_port(g_handle, 1, &g_ctrl_output);
    g_desc->connect_port(g_handle, 2, &g_ctrl_hi_thru);
    g_desc->connect_port(g_handle, 3, &g_ctrl_hi_band);
    g_desc->connect_port(g_handle, 4, &g_ctrl_envelope);
    g_desc->connect_port(g_handle, 5, &g_ctrl_filter_q);
    g_desc->connect_port(g_handle, 6, &g_ctrl_mid_freq);
    g_desc->connect_port(g_handle, 7, &g_ctrl_quality);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_mod_in(float v) { g_ctrl_mod_in = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_mod_in()        { return g_ctrl_mod_in; }
...
EMSCRIPTEN_KEEPALIVE void  shim_set_hi_thru(float v) { g_ctrl_hi_thru = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_hi_thru()        { return g_ctrl_hi_thru; }
EMSCRIPTEN_KEEPALIVE void  shim_set_hi_band(float v) { g_ctrl_hi_band = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_hi_band()        { return g_ctrl_hi_band; }
...
EMSCRIPTEN_KEEPALIVE void  shim_set_filter_q(float v) { g_ctrl_filter_q = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_filter_q()        { return g_ctrl_filter_q; }
EMSCRIPTEN_KEEPALIVE void  shim_set_mid_freq(float v) { g_ctrl_mid_freq = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_mid_freq()        { return g_ctrl_mid_freq; }
