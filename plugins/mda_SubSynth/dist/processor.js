import createMDA_SubSynthPlugin from './MDA_SubSynth.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"type":"_shim_set_type","level":"_shim_set_level","tune":"_shim_set_tune","dry_mix":"_shim_set_dry_mix","thresh":"_shim_set_thresh","release":"_shim_set_release"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_SubSynthPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_SubSynth', WadspProcessor);
