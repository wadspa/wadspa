...
static float g_ctrl_type = 0.5f;
static float g_ctrl_level = 50.0f;
static float g_ctrl_tune = 198.0f;
static float g_ctrl_dry_mix = 50.0f;
static float g_ctrl_thresh = -60.0f;
static float g_ctrl_release = 1.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/SubSynth";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 0, &g_ctrl_type);
    g_desc->connect_port(g_handle, 1, &g_ctrl_level);
    g_desc->connect_port(g_handle, 2, &g_ctrl_tune);
    g_desc->connect_port(g_handle, 3, &g_ctrl_dry_mix);
    g_desc->connect_port(g_handle, 4, &g_ctrl_thresh);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_type(float v) { g_ctrl_type = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_type()        { return g_ctrl_type; }
...
EMSCRIPTEN_KEEPALIVE void  shim_set_dry_mix(float v) { g_ctrl_dry_mix = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_dry_mix()        { return g_ctrl_dry_mix; }
