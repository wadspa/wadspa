...
static float g_cv_freq[BLOCK_SIZE];
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    g_desc->connect_port(g_handle, 1, g_cv_freq);
    g_desc->connect_port(g_handle, 2, g_cv_exp_fm);
    g_desc->connect_port(g_handle, 3, g_cv_lin_fm);
...
EMSCRIPTEN_KEEPALIVE void shim_run(unsigned long count) {
    for (unsigned long _i = 0; _i < count && _i < BLOCK_SIZE; _i++) g_cv_freq[_i] = g_ctrl_freq;
