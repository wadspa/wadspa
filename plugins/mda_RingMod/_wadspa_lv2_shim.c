...
static float g_ctrl_freq = 1000.0f;
static float g_ctrl_fine = 0.0f;
static float g_ctrl_feedback = 0.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/RingMod";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 2, &g_ctrl_feedback);
