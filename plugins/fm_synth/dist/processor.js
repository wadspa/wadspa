import createFM_SynthPlugin from './FM_Synth.js';

let mod = null;
const inPtrs  = [];
const outPtrs = [0, 0];
const SETTERS = {"mod_ratio":"_shim_set_mod_ratio","mod_index":"_shim_set_mod_index","mod_decay":"_shim_set_mod_decay","attack":"_shim_set_attack","decay":"_shim_set_decay","sustain":"_shim_set_sustain","release":"_shim_set_release","gain":"_shim_set_gain"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createFM_SynthPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-FM_Synth', WadspProcessor);
