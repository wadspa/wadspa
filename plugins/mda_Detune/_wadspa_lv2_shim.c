...
static float g_ctrl_detune = 2.4f;
static float g_ctrl_mix = 50.0f;
static float g_ctrl_output = 0.0f;
static float g_ctrl_latency = 0.5f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/Detune";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 0, &g_ctrl_detune);
    g_desc->connect_port(g_handle, 1, &g_ctrl_mix);
    g_desc->connect_port(g_handle, 2, &g_ctrl_output);
    g_desc->connect_port(g_handle, 3, &g_ctrl_latency);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_detune(float v) { g_ctrl_detune = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_detune()        { return g_ctrl_detune; }
...
EMSCRIPTEN_KEEPALIVE void  shim_set_latency(float v) { g_ctrl_latency = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_latency()        { return g_ctrl_latency; }
