...
static float g_ctrl_size = 7.0f;
static float g_ctrl_hf_damp = 70.0f;
static float g_ctrl_mix = 90.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/Ambience";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 0, &g_ctrl_size);
    g_desc->connect_port(g_handle, 1, &g_ctrl_hf_damp);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_size(float v) { g_ctrl_size = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_size()        { return g_ctrl_size; }
EMSCRIPTEN_KEEPALIVE void  shim_set_hf_damp(float v) { g_ctrl_hf_damp = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_hf_damp()        { return g_ctrl_hf_damp; }
