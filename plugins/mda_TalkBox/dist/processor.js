import createMDA_TalkBoxPlugin from './MDA_TalkBox.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"wet":"_shim_set_wet","dry":"_shim_set_dry","carrier":"_shim_set_carrier","quality":"_shim_set_quality"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_TalkBoxPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_TalkBox', WadspProcessor);
