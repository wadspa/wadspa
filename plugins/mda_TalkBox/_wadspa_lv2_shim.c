...
static float g_ctrl_wet = 100.0f;
static float g_ctrl_dry = 0.0f;
static float g_ctrl_carrier = 0.0f;
static float g_ctrl_quality = 100.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/TalkBox";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 0, &g_ctrl_wet);
    g_desc->connect_port(g_handle, 1, &g_ctrl_dry);
    g_desc->connect_port(g_handle, 2, &g_ctrl_carrier);
    g_desc->connect_port(g_handle, 3, &g_ctrl_quality);
    g_desc->connect_port(g_handle, 4, g_in_left_in);
    g_desc->connect_port(g_handle, 5, g_in_right_in);
    g_desc->connect_port(g_handle, 6, g_out_left_out);
    g_desc->connect_port(g_handle, 7, g_out_right_out);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_wet(float v) { g_ctrl_wet = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_wet()        { return g_ctrl_wet; }
EMSCRIPTEN_KEEPALIVE void  shim_set_dry(float v) { g_ctrl_dry = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_dry()        { return g_ctrl_dry; }
EMSCRIPTEN_KEEPALIVE void  shim_set_carrier(float v) { g_ctrl_carrier = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_carrier()        { return g_ctrl_carrier; }
