...
static float g_ctrl_muffle = 0.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/Overdrive";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 0, &g_ctrl_drive);
    g_desc->connect_port(g_handle, 1, &g_ctrl_muffle);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_muffle(float v) { g_ctrl_muffle = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_muffle()        { return g_ctrl_muffle; }
