...
static float g_ctrl_loudness = 0.7f;
static float g_ctrl_output = 0.5f;
static float g_ctrl_link = 0.35f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/Loudness";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 0, &g_ctrl_loudness);
    g_desc->connect_port(g_handle, 1, &g_ctrl_output);
    g_desc->connect_port(g_handle, 2, &g_ctrl_link);
    g_desc->connect_port(g_handle, 3, g_in_left_in);
    g_desc->connect_port(g_handle, 4, g_in_right_in);
    g_desc->connect_port(g_handle, 5, g_out_left_out);
    g_desc->connect_port(g_handle, 6, g_out_right_out);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_loudness(float v) { g_ctrl_loudness = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_loudness()        { return g_ctrl_loudness; }
...
EMSCRIPTEN_KEEPALIVE void  shim_set_link(float v) { g_ctrl_link = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_link()        { return g_ctrl_link; }
