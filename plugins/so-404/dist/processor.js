import createSO404_Bass_SynthesizerPlugin from './SO404_Bass_Synthesizer.js';

let mod = null;
const inPtrs  = [];
const outPtrs = [0];
const SETTERS = {"controlmode":"_shim_set_controlmode","volume":"_shim_set_volume","cutoff":"_shim_set_cutoff","resonance":"_shim_set_resonance","envelope":"_shim_set_envelope","portamento":"_shim_set_portamento","release":"_shim_set_release","channel":"_shim_set_channel"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createSO404_Bass_SynthesizerPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-SO404_Bass_Synthesizer', WadspProcessor);
