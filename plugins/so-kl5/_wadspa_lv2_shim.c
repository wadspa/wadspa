...
static float g_ctrl_sustain = 0.0f;
static float g_ctrl_resonance = 0.625f;
static float g_ctrl_cutoff = 0.1725f;
static float g_ctrl_attack = 0.01125f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "urn:50m30n3:plugins:SO-kl5";
      for (int _i = 0; ; _i++) {
...
#pragma GCC diagnostic pop
    ...
    g_desc->connect_port(g_handle, 3, &g_ctrl_sustain);
    ...
    g_desc->connect_port(g_handle, 7, &g_ctrl_volume);
    g_desc->connect_port(g_handle, 8, &g_ctrl_channel);
