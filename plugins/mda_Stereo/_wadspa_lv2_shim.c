...
static float g_ctrl_width = 56.0f;
static float g_ctrl_delay = 22.43f;
static float g_ctrl_balance = 0.0f;
static float g_ctrl_mod = 0.0f;
static float g_ctrl_rate = 50.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/Stereo";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 0, &g_ctrl_width);
    g_desc->connect_port(g_handle, 1, &g_ctrl_delay);
    g_desc->connect_port(g_handle, 2, &g_ctrl_balance);
    g_desc->connect_port(g_handle, 3, &g_ctrl_mod);
    g_desc->connect_port(g_handle, 4, &g_ctrl_rate);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_width(float v) { g_ctrl_width = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_width()        { return g_ctrl_width; }
...
EMSCRIPTEN_KEEPALIVE void  shim_set_balance(float v) { g_ctrl_balance = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_balance()        { return g_ctrl_balance; }
EMSCRIPTEN_KEEPALIVE void  shim_set_mod(float v) { g_ctrl_mod = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_mod()        { return g_ctrl_mod; }
