import createMDA_StereoPlugin from './MDA_Stereo.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"width":"_shim_set_width","delay":"_shim_set_delay","balance":"_shim_set_balance","mod":"_shim_set_mod","rate":"_shim_set_rate"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_StereoPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_Stereo', WadspProcessor);
