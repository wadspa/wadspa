...
static float g_ctrl_rate = 0.08f;
static float g_ctrl_depth = 20.0f;
static float g_ctrl_mix = 47.0f;
static float g_ctrl_feedback = -40.0f;
static float g_ctrl_depth_mod = 100.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/ThruZero";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 0, &g_ctrl_rate);
    g_desc->connect_port(g_handle, 1, &g_ctrl_depth);
    g_desc->connect_port(g_handle, 2, &g_ctrl_mix);
    g_desc->connect_port(g_handle, 3, &g_ctrl_feedback);
    g_desc->connect_port(g_handle, 4, &g_ctrl_depth_mod);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_depth(float v) { g_ctrl_depth = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_depth()        { return g_ctrl_depth; }
...
EMSCRIPTEN_KEEPALIVE void  shim_set_depth_mod(float v) { g_ctrl_depth_mod = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_depth_mod()        { return g_ctrl_depth_mod; }
