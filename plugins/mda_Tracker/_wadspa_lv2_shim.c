...
static float g_ctrl_dynamics = 100.0f;
static float g_ctrl_mix = 100.0f;
static float g_ctrl_glide = 97.0f;
static float g_ctrl_transpose = 0.0f;
static float g_ctrl_maximum = 6500.0f;
static float g_ctrl_trigger = -30.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/Tracker";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 1, &g_ctrl_dynamics);
    g_desc->connect_port(g_handle, 2, &g_ctrl_mix);
    g_desc->connect_port(g_handle, 3, &g_ctrl_glide);
    g_desc->connect_port(g_handle, 4, &g_ctrl_transpose);
    g_desc->connect_port(g_handle, 5, &g_ctrl_maximum);
    g_desc->connect_port(g_handle, 6, &g_ctrl_trigger);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_transpose(float v) { g_ctrl_transpose = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_transpose()        { return g_ctrl_transpose; }
EMSCRIPTEN_KEEPALIVE void  shim_set_maximum(float v) { g_ctrl_maximum = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_maximum()        { return g_ctrl_maximum; }
