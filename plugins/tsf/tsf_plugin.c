 ...
 * synthesis as a wadspa instrument.  Call _shim_load_sf2(ptr, size) with
...
EMSCRIPTEN_KEEPALIVE void _shim_init(int sr) {
    g_sr = sr;
}

EMSCRIPTEN_KEEPALIVE void _shim_load_sf2(const void* data, int size) {
...
EMSCRIPTEN_KEEPALIVE void _shim_set_gain(float v) {
    g_gain = v;
    if (g_tsf) tsf_set_volume(g_tsf, v);
}

EMSCRIPTEN_KEEPALIVE void _shim_midi_note_on(int ch, int note, int vel) {
    if (g_tsf) tsf_channel_note_on(g_tsf, ch, note, vel / 127.0f);
}

EMSCRIPTEN_KEEPALIVE void _shim_midi_note_off(int ch, int note) {
    if (g_tsf) tsf_channel_note_off(g_tsf, ch, note);
}

EMSCRIPTEN_KEEPALIVE void _shim_midi_cc(int ch, int cc, int val) {
    if (g_tsf) tsf_channel_midi_control(g_tsf, ch, cc, val);
}

EMSCRIPTEN_KEEPALIVE void _shim_midi_pitch_bend(int ch, int bend) {
    /* bend: -8192..+8191 -> TSF wheel: 0..16383 */
    if (g_tsf) tsf_channel_set_pitchwheel(g_tsf, ch, bend + 8192);
}

EMSCRIPTEN_KEEPALIVE void _shim_midi_program_change(int ch, int program) {
    /* ch 9 = drums in General MIDI */
    if (g_tsf) tsf_channel_set_presetnumber(g_tsf, ch, program, ch == 9);
}

EMSCRIPTEN_KEEPALIVE void _shim_run(int frames) {
    if (!g_tsf) {
...
EMSCRIPTEN_KEEPALIVE float* _shim_output_buf_out_l() { return g_buf; }
EMSCRIPTEN_KEEPALIVE float* _shim_output_buf_out_r() { return g_buf + BLOCK_SIZE; }
