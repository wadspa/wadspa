...
static float g_ctrl_pan = 0.0f;
static float g_ctrl_auto = 309.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/RoundPan";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 0, &g_ctrl_pan);
    g_desc->connect_port(g_handle, 1, &g_ctrl_auto);
    g_desc->connect_port(g_handle, 2, g_in_left_in);
    g_desc->connect_port(g_handle, 3, g_in_right_in);
    g_desc->connect_port(g_handle, 4, g_out_left_out);
    g_desc->connect_port(g_handle, 5, g_out_right_out);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_pan(float v) { g_ctrl_pan = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_pan()        { return g_ctrl_pan; }
EMSCRIPTEN_KEEPALIVE void  shim_set_auto(float v) { g_ctrl_auto = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_auto()        { return g_ctrl_auto; }
