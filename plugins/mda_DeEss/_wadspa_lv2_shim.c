...
static float g_ctrl_thresh = -30.0f;
static float g_ctrl_freq = 7600.0f;
static float g_ctrl_hf_drive = 0.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/DeEss";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 2, &g_ctrl_hf_drive);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_hf_drive(float v) { g_ctrl_hf_drive = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_hf_drive()        { return g_ctrl_hf_drive; }
