import createMDA_DeessPlugin from './MDA_Deess.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"thresh":"_shim_set_thresh","freq":"_shim_set_freq","hf_drive":"_shim_set_hf_drive"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_DeessPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_Deess', WadspProcessor);
