...
static float g_ctrl_sections = 1.0f;
static float g_ctrl_freq = 0.0f;
static float g_ctrl_exp_fm_gain = 0.0f;
static float g_ctrl_lin_fm_gain = 0.0f;
static float g_ctrl_fb_gain = 0.0f;
static float g_ctrl_out_mix = 0.0f;
static float g_cv_fm[BLOCK_SIZE];
static float g_cv_exp_fm[BLOCK_SIZE];
static float g_cv_lin_fm[BLOCK_SIZE];
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://drobilla.net/plugins/fomp/cs_phaser1";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 4, g_cv_lin_fm);
    g_desc->connect_port(g_handle, 5, &g_ctrl_in_gain);
    g_desc->connect_port(g_handle, 6, &g_ctrl_sections);
    g_desc->connect_port(g_handle, 7, &g_ctrl_freq);
    g_desc->connect_port(g_handle, 8, &g_ctrl_exp_fm_gain);
    g_desc->connect_port(g_handle, 9, &g_ctrl_lin_fm_gain);
    g_desc->connect_port(g_handle, 10, &g_ctrl_fb_gain);
    g_desc->connect_port(g_handle, 11, &g_ctrl_out_mix);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_sections(float v) { g_ctrl_sections = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_sections()        { return g_ctrl_sections; }
...
EMSCRIPTEN_KEEPALIVE void  shim_set_fb_gain(float v) { g_ctrl_fb_gain = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_fb_gain()        { return g_ctrl_fb_gain; }
...
EMSCRIPTEN_KEEPALIVE void shim_run(unsigned long count) {
    ...
    for (unsigned long _i = 0; _i < count && _i < BLOCK_SIZE; _i++) g_cv_lin_fm[_i] = g_ctrl_lin_fm;
