...
static float g_ctrl_cutoff = 0.5f;
static float g_ctrl_attack = 0.01f;
static float g_ctrl_decay = 0.1f;
static float g_ctrl_sustain = 0.7f;
static float g_ctrl_release = 0.3f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "https://wadspa.org/plugins/wadspa_synth";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 3, &g_ctrl_cutoff);
    g_desc->connect_port(g_handle, 4, &g_ctrl_attack);
    g_desc->connect_port(g_handle, 5, &g_ctrl_decay);
    g_desc->connect_port(g_handle, 6, &g_ctrl_sustain);
    g_desc->connect_port(g_handle, 7, &g_ctrl_release);
    g_desc->connect_port(g_handle, 8, &g_ctrl_gain);
