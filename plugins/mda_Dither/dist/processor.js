import createMDA_DitherPlugin from './MDA_Dither.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"word_len":"_shim_set_word_len","dither":"_shim_set_dither","dith_amp":"_shim_set_dith_amp","dc_trim":"_shim_set_dc_trim","zoom":"_shim_set_zoom"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_DitherPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_Dither', WadspProcessor);
