import createMDA_DubDelayPlugin from './MDA_DubDelay.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"delay":"_shim_set_delay","feedback":"_shim_set_feedback","fb_tone":"_shim_set_fb_tone","lfo_depth":"_shim_set_lfo_depth","lfo_rate":"_shim_set_lfo_rate","fx_mix":"_shim_set_fx_mix","output":"_shim_set_output"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_DubDelayPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_DubDelay', WadspProcessor);
