...
static float g_ctrl_delay = 460.0f;
static float g_ctrl_feedback = 55.0f;
static float g_ctrl_fb_tone = 0.0f;
static float g_ctrl_lfo_depth = 30.0f;
static float g_ctrl_lfo_rate = 0.05f;
static float g_ctrl_fx_mix = 50.0f;
...
EMSCRIPTEN_KEEPALIVE void shim_init(unsigned long sample_rate) {
    ...
    { const char *_uri = "http://moddevices.com/plugins/mda/DubDelay";
      for (int _i = 0; ; _i++) {
    ...
    g_desc->connect_port(g_handle, 0, &g_ctrl_delay);
    g_desc->connect_port(g_handle, 1, &g_ctrl_feedback);
    g_desc->connect_port(g_handle, 2, &g_ctrl_fb_tone);
    g_desc->connect_port(g_handle, 3, &g_ctrl_lfo_depth);
    g_desc->connect_port(g_handle, 4, &g_ctrl_lfo_rate);
...
EMSCRIPTEN_KEEPALIVE void  shim_set_lfo_depth(float v) { g_ctrl_lfo_depth = v; }
EMSCRIPTEN_KEEPALIVE float shim_get_lfo_depth()        { return g_ctrl_lfo_depth; }
