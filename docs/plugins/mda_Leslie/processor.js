import createMDA_LesliePlugin from './MDA_Leslie.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"mode":"_shim_set_mode","lo_width":"_shim_set_lo_width","lo_throb":"_shim_set_lo_throb","hi_width":"_shim_set_hi_width","hi_depth":"_shim_set_hi_depth","hi_throb":"_shim_set_hi_throb","x_over":"_shim_set_x_over","output":"_shim_set_output","speed":"_shim_set_speed"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_LesliePlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_Leslie', WadspProcessor);
