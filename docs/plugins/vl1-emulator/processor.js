import createVL1emulatorPlugin from './VL1emulator.js';

let mod = null;
const inPtrs  = [];
const outPtrs = [0, 0];
const SETTERS = {"mode":"_shim_set_mode","volume":"_shim_set_volume","balance":"_shim_set_balance","octave":"_shim_set_octave","tune":"_shim_set_tune","sound":"_shim_set_sound","attack":"_shim_set_attack","decay":"_shim_set_decay","sustain_level":"_shim_set_sustain_level","sustain_time":"_shim_set_sustain_time","release":"_shim_set_release","vibrato":"_shim_set_vibrato","tremolo":"_shim_set_tremolo","tempo":"_shim_set_tempo"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createVL1emulatorPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-VL1emulator', WadspProcessor);
