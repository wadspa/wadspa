import createTAP_Mono_DynamicsPlugin from './TAP_Mono_Dynamics.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0];
const SETTERS = {"attack":"_shim_set_attack","release":"_shim_set_release","offset":"_shim_set_offset","makeup":"_shim_set_makeup","function":"_shim_set_function"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createTAP_Mono_DynamicsPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-TAP_Mono_Dynamics', WadspProcessor);
