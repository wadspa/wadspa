import createTAP_ReflectorPlugin from './TAP_Reflector.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0];
const SETTERS = {"fragment":"_shim_set_fragment","dry":"_shim_set_dry","wet":"_shim_set_wet"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createTAP_ReflectorPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-TAP_Reflector', WadspProcessor);
