import createMDA_VocInputPlugin from './MDA_VocInput.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"tracking":"_shim_set_tracking","pitch":"_shim_set_pitch","breath":"_shim_set_breath","s_thresh":"_shim_set_s_thresh","max_freq":"_shim_set_max_freq"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_VocInputPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_VocInput', WadspProcessor);
