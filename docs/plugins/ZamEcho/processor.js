import createZamEchoPlugin from './ZamEcho.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0];
const SETTERS = {"echo":"_shim_set_echo"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createZamEchoPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-ZamEcho', WadspProcessor);
