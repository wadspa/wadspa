import createMDA_RePsychoPlugin from './MDA_RePsycho.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"tune":"_shim_set_tune","fine":"_shim_set_fine","decay":"_shim_set_decay","thresh":"_shim_set_thresh","hold":"_shim_set_hold","mix":"_shim_set_mix","quality":"_shim_set_quality"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_RePsychoPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_RePsycho', WadspProcessor);
