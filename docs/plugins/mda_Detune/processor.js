import createMDA_DetunePlugin from './MDA_Detune.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"detune":"_shim_set_detune","mix":"_shim_set_mix","output":"_shim_set_output","latency":"_shim_set_latency"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_DetunePlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_Detune', WadspProcessor);
