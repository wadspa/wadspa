import createCS_Phaser_1Plugin from './CS_Phaser_1.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0];
const SETTERS = {"fm":"_shim_set_fm","exp_fm":"_shim_set_exp_fm","lin_fm":"_shim_set_lin_fm","in_gain":"_shim_set_in_gain","sections":"_shim_set_sections","freq":"_shim_set_freq","exp_fm_gain":"_shim_set_exp_fm_gain","lin_fm_gain":"_shim_set_lin_fm_gain","fb_gain":"_shim_set_fb_gain","out_mix":"_shim_set_out_mix"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createCS_Phaser_1Plugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-CS_Phaser_1', WadspProcessor);
