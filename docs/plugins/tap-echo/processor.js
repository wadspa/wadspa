import createTAP_Stereo_EchoPlugin from './TAP_Stereo_Echo.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"ldelay":"_shim_set_ldelay","lfeedback":"_shim_set_lfeedback","rhaasdelay":"_shim_set_rhaasdelay","rhaasfeedback":"_shim_set_rhaasfeedback","lecholevel":"_shim_set_lecholevel","recholevel":"_shim_set_recholevel","dryLevel":"_shim_set_dryLevel","crossmode":"_shim_set_crossmode","haaseffect":"_shim_set_haaseffect","swapoutputs":"_shim_set_swapoutputs"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createTAP_Stereo_EchoPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
                    mod._shim_init(sampleRate);
                    inPtrs[0]  = mod._shim_input_buf_inputleft() >> 2;
                    inPtrs[1]  = mod._shim_input_buf_inputright() >> 2;
                    outPtrs[0] = mod._shim_output_buf_outputleft() >> 2;
                    outPtrs[1] = mod._shim_output_buf_outputright() >> 2;
...
registerProcessor('wadspa-TAP_Stereo_Echo', WadspProcessor);
