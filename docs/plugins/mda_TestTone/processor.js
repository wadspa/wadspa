import createMDA_TestTonePlugin from './MDA_TestTone.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"mode":"_shim_set_mode","level":"_shim_set_level","channel":"_shim_set_channel","f1":"_shim_set_f1","f2":"_shim_set_f2","sweep":"_shim_set_sweep","thru":"_shim_set_thru","zero_db":"_shim_set_zero_db"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_TestTonePlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_TestTone', WadspProcessor);
