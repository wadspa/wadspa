import createTAP_TubewarmthPlugin from './TAP_Tubewarmth.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0];
const SETTERS = {"drive":"_shim_set_drive","blend":"_shim_set_blend"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createTAP_TubewarmthPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-TAP_Tubewarmth', WadspProcessor);
