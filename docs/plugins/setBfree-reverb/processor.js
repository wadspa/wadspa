import createsetBfree_Organ_ReverbPlugin from './setBfree_Organ_Reverb.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0];
const SETTERS = {"mix":"_shim_set_mix","gain_in":"_shim_set_gain_in"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createsetBfree_Organ_ReverbPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-setBfree_Organ_Reverb', WadspProcessor);
