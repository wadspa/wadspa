import createTAP_TremoloPlugin from './TAP_Tremolo.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0];
const SETTERS = {"frequency":"_shim_set_frequency","depth":"_shim_set_depth","gain":"_shim_set_gain"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createTAP_TremoloPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
                    mod._shim_init(sampleRate);
                    inPtrs[0]  = mod._shim_input_buf_input_0() >> 2;
                    outPtrs[0] = mod._shim_output_buf_output_0() >> 2;
...
registerProcessor('wadspa-TAP_Tremolo', WadspProcessor);
