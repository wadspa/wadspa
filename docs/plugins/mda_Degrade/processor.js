import createMDA_DegradePlugin from './MDA_Degrade.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"headroom":"_shim_set_headroom","quant":"_shim_set_quant","rate":"_shim_set_rate","integrator_sr":"_shim_set_integrator_sr","post_filt":"_shim_set_post_filt","non_lin":"_shim_set_non_lin","even_odd":"_shim_set_even_odd","output":"_shim_set_output"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_DegradePlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_Degrade', WadspProcessor);
