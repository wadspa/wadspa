import createZamCompX2Plugin from './ZamCompX2.js';

let mod = null;
const inPtrs  = [0, 0, 0];
const outPtrs = [0, 0];
const SETTERS = {"att":"_shim_set_att","rel":"_shim_set_rel","kn":"_shim_set_kn","rat":"_shim_set_rat","thr":"_shim_set_thr","mak":"_shim_set_mak","slew":"_shim_set_slew","stereodet":"_shim_set_stereodet","sidechain":"_shim_set_sidechain"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createZamCompX2Plugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-ZamCompX2', WadspProcessor);
