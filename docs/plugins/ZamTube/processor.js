import createZamTubePlugin from './ZamTube.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0];
const SETTERS = {"tubedrive":"_shim_set_tubedrive","bass":"_shim_set_bass","mids":"_shim_set_mids","treb":"_shim_set_treb","tonestack":"_shim_set_tonestack","gain":"_shim_set_gain","insane":"_shim_set_insane"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createZamTubePlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-ZamTube', WadspProcessor);
