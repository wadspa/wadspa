...
static inline void eq_set_params(biquad *f, bq_t fc, bq_t gain, bq_t bw,
			  bq_t fs)
{
	bq_t w = 2.0 * M_PI * LIMIT(fc, 1.0, fs/2.0) / fs;
	bq_t cw = cosf(w);
	bq_t sw = sinf(w);
	bq_t J = pow(10.0f, gain * 0.025);
	bq_t g = sw * sinhf(LN_2_2 * LIMIT(bw, 0.0001, 4.0) * w / sw);
	bq_t a0r = 1.0 / (1.0 + (g / J));
			  ...
			  bq_t fs)
{
	...
	bq_t cw = cosf(w);
	bq_t sw = sinf(w);
	bq_t A = powf(10.0f, gain * 0.025);
	bq_t b = sqrt(((1.0 + A * A) / LIMIT(slope, 0.0001, 1.0)) - ((A -
					1.0) * (A - 1.0)));
	bq_t apc = cw * (A + 1.0);
	bq_t amc = cw * (A - 1.0);
	bq_t bs = b * sw;
	bq_t a0r = 1.0 / (A + 1.0 + amc + bs);
