import createTAP_DeEsserPlugin from './TAP_DeEsser.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0];
const SETTERS = {"thresholdlevel":"_shim_set_thresholdlevel","frequency":"_shim_set_frequency","sidechainfilter":"_shim_set_sidechainfilter","monitor":"_shim_set_monitor"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createTAP_DeEsserPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-TAP_DeEsser', WadspProcessor);
