import createMoog_HighPass_Filter_1Plugin from './Moog_HighPass_Filter_1.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0];
const SETTERS = {"fm":"_shim_set_fm","exp_fm":"_shim_set_exp_fm","in_gain":"_shim_set_in_gain","freq":"_shim_set_freq","exp_gm_gain":"_shim_set_exp_gm_gain","out_gain":"_shim_set_out_gain"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMoog_HighPass_Filter_1Plugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-Moog_HighPass_Filter_1', WadspProcessor);
