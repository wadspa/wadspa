import createMDA_TransientPlugin from './MDA_Transient.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"attack":"_shim_set_attack","release":"_shim_set_release","output":"_shim_set_output","filter":"_shim_set_filter","att_hold":"_shim_set_att_hold","rel_hold":"_shim_set_rel_hold"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_TransientPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_Transient', WadspProcessor);
