import createTAP_Scaling_LimiterPlugin from './TAP_Scaling_Limiter.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0];
const SETTERS = {"limitlevel":"_shim_set_limitlevel","outputvolume":"_shim_set_outputvolume"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createTAP_Scaling_LimiterPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-TAP_Scaling_Limiter', WadspProcessor);
