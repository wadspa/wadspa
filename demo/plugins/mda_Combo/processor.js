import createMDA_ComboPlugin from './MDA_Combo.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"model":"_shim_set_model","drive":"_shim_set_drive","bias":"_shim_set_bias","output":"_shim_set_output","stereo":"_shim_set_stereo","hpf_freq":"_shim_set_hpf_freq","hpf_reso":"_shim_set_hpf_reso"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_ComboPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_Combo', WadspProcessor);
