import createMDA_RezFilterPlugin from './MDA_RezFilter.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"freq":"_shim_set_freq","res":"_shim_set_res","output":"_shim_set_output","env_vcf":"_shim_set_env_vcf","attack":"_shim_set_attack","release":"_shim_set_release","lfo_vcf":"_shim_set_lfo_vcf","lfo_rate":"_shim_set_lfo_rate","trigger":"_shim_set_trigger","max_freq":"_shim_set_max_freq"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_RezFilterPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_RezFilter', WadspProcessor);
