import createTAP_PinkFractal_NoisePlugin from './TAP_PinkFractal_Noise.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0];
const SETTERS = {"hurst":"_shim_set_hurst","signal":"_shim_set_signal","noise":"_shim_set_noise"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createTAP_PinkFractal_NoisePlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-TAP_PinkFractal_Noise', WadspProcessor);
