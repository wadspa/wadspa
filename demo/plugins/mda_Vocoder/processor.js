import createMDA_VocoderPlugin from './MDA_Vocoder.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"mod_in":"_shim_set_mod_in","output":"_shim_set_output","hi_thru":"_shim_set_hi_thru","hi_band":"_shim_set_hi_band","envelope":"_shim_set_envelope","filter_q":"_shim_set_filter_q","mid_freq":"_shim_set_mid_freq","quality":"_shim_set_quality"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_VocoderPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_Vocoder', WadspProcessor);
