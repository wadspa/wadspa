import createZamDelayPlugin from './ZamDelay.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0];
const SETTERS = {"inv":"_shim_set_inv","time":"_shim_set_time","sync":"_shim_set_sync","lpf":"_shim_set_lpf","div":"_shim_set_div","gain":"_shim_set_gain","drywet":"_shim_set_drywet","feedb":"_shim_set_feedb"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createZamDelayPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-ZamDelay', WadspProcessor);
