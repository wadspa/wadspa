import createMDA_ThruZeroPlugin from './MDA_ThruZero.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"rate":"_shim_set_rate","depth":"_shim_set_depth","mix":"_shim_set_mix","feedback":"_shim_set_feedback","depth_mod":"_shim_set_depth_mod"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_ThruZeroPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_ThruZero', WadspProcessor);
