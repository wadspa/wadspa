import createMDA_LimiterPlugin from './MDA_Limiter.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"thresh":"_shim_set_thresh","output":"_shim_set_output","release":"_shim_set_release","attack":"_shim_set_attack","knee":"_shim_set_knee"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_LimiterPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_Limiter', WadspProcessor);
