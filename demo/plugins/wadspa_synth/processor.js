import createwadspa_synthPlugin from './wadspa_synth.js';

let mod = null;
const inPtrs  = [];
const outPtrs = [0, 0];
const SETTERS = {"cutoff":"_shim_set_cutoff","attack":"_shim_set_attack","decay":"_shim_set_decay","sustain":"_shim_set_sustain","release":"_shim_set_release","gain":"_shim_set_gain"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createwadspa_synthPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-wadspa_synth', WadspProcessor);
