import createMDA_DynamicsPlugin from './MDA_Dynamics.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"thresh":"_shim_set_thresh","ratio":"_shim_set_ratio","output":"_shim_set_output","attack":"_shim_set_attack","release":"_shim_set_release","limiter":"_shim_set_limiter","gate_thr":"_shim_set_gate_thr","gate_att":"_shim_set_gate_att","gate_rel":"_shim_set_gate_rel","mix":"_shim_set_mix"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_DynamicsPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_Dynamics', WadspProcessor);
