import createMDA_TrackerPlugin from './MDA_Tracker.js';

let mod = null;
const inPtrs  = [0, 0];
const outPtrs = [0, 0];
const SETTERS = {"mode":"_shim_set_mode","dynamics":"_shim_set_dynamics","mix":"_shim_set_mix","glide":"_shim_set_glide","transpose":"_shim_set_transpose","maximum":"_shim_set_maximum","trigger":"_shim_set_trigger","output":"_shim_set_output"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createMDA_TrackerPlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-MDA_Tracker', WadspProcessor);
