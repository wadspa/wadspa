import createplatePlugin from './plate.js';

let mod = null;
const inPtrs  = [0];
const outPtrs = [0, 0];
const SETTERS = {"0":"_shim_set_reverb_time","1":"_shim_set_damping","2":"_shim_set_dry_wet_mix"};
...
class WadspProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async ({ data }) => {
            if (data.type === 'setup') {
                try {
                    mod = await createplatePlugin({ wasmBinary: data.wasm, locateFile: (p, d) => d + p });
...
registerProcessor('wadspa-plate', WadspProcessor);
