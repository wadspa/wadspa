...
const cssChecks = [
  ...
  ['no click-to-position knob jump', /const beginDrag = \(startEvent[\s\S]*cancelRelativePointerEvent\(startEvent\);[\s\S]*const startValue = Number\(control\.value\)[\s\S]*gestureTravelPastDeadzone\(rawTravel,\s*moveEvent\.shiftKey \? 1 : 3\)[\s\S]*startValue \+ \(travel \/ pixels\) \* span/],
