...
const cssChecks = [
  ...
  ['released knob motion ignored', /if \('buttons' in moveEvent && moveEvent\.buttons === 0\) return;/],
  ['knob drag starts after deadzone', /let dragOriginY = startY;[\s\S]*let dragEngaged = false;[\s\S]*if \(!dragEngaged\) \{[\s\S]*const initialTravel = -\(moveEvent\.clientY - startY\);[\s\S]*gestureTravelPastDeadzone\(initialTravel,\s*deadzone\) === 0[\s\S]*dragEngaged = true;[\s\S]*dragOriginY = moveEvent\.clientY;[\s\S]*return;/],
  ...
  ['no click-to-position knob jump', /const beginDrag = \(startEvent[\s\S]*cancelRelativePointerEvent\(startEvent\);[\s\S]*const startValue = Number\(control\.value\)[\s\S]*let dragEngaged = false;[\s\S]*if \(!dragEngaged\)[\s\S]*return;[\s\S]*const travel = -dy;[\s\S]*startValue \+ \(travel \/ pixels\) \* span/],
  ...
  ['knob tap highlight disabled', /\.ctrl-row\[data-widget="knob"\] \.control-stack,[\s\S]*-webkit-tap-highlight-color:\s*transparent;/s],
